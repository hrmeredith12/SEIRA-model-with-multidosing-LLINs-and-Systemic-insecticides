...
test <- array(0, c( x, y, z+1))
test[,,1:2] <- periodUse[,,1]
test[,,3:7] <- periodUse[,,2:6]

# Plotting contour maps and overlaying data

setwd("C:/Users/Hannah/Desktop/LSHTM/Models/R_modeling/SEIRA")

dataset.df <- data.frame(read_excel("20180810_cmax_halflife_LC50_forR.xlsx"))
dataset.df$host <- as.factor(dataset.df$host)
dataset.df$drug <- as.factor(dataset.df$drug)
dataset.df$route <- as.factor(dataset.df$route)
dataset.df$loopID <- ifelse(is.na(dataset.df$cmaxMean),0,
                         ifelse(is.na(dataset.df$halflifeMean),0,
                                ifelse(dataset.df$drug == 'eprinomectin',1, 
                                       ifelse (dataset.df$drug == 'ivermectin',2,
                                               ifelse (dataset.df$drug == 'fluralaner',3,
                                                       ifelse (dataset.df$drug == 'doramectin',4,
                                                               ifelse (dataset.df$drug == 'afoxalaner',5,
                                                                       ifelse (dataset.df$drug == 'spinosad',6,
                                                                               ifelse (dataset.df$drug == 'moxidectin',7,0)))))))))
...
colors <- as.factor(gg_color_hue(8))
names(colors) <- levels(dataset.df$host)
