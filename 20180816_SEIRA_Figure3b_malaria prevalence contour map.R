## Reproduces Figure 3b contour map of malaria prevalence reduction for a given half life and cmax 
# Assumes p_H = 0.5, m = 20, LC50 = 7.5, period = montly, and livestock treatment + LLINs
# by Hannah Meredith
...
relative_malaria <- array(someData, c( x, y, z)) #matrix(,nrow = size(halflife_d_range, 2), ncol = size(cmax_range, 2))
...
for (h in 1) {#(h in 1:z) {
   for (j in 1:x) {
     for (k in 1:y) {
      for (i in 2) {#(i in size(period_D_range, 2):1) {
        ...
        relative_malaria[j,k,i] <- (infected_net_steadyState - tail(infected_all,n=1))/infected_net_steadyState * 100
        relative_malaria [relative_malaria < 0] = 0
      

        print(c(i,k,j,h,relative_malaria[j,k,i]))
...
for (i in 2){#(i in 1:7){
  data <- round(relative_malaria[,,i])#round(periodUse[,,i])
  colnames(data) <- cmax_range
  rownames(data) <- halflife_d_range
  longData <- melt(data)
  longData.df <- data.frame(longData)
  colnames(longData.df) <- c("halflife", "cmax", "mal_prev")
  par(pty = "s") # axis
  ...
  p1 <- p +
    ...
    coord_cartesian(xlim = c(0.1, 100), ylim = c(1, 10000), expand=FALSE) 
