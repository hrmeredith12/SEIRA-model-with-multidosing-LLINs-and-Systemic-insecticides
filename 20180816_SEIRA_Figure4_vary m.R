## Reproduces Figure 4 subplots that vary m and coverage type:
# Effect of Half life, C_max, LC50, and dosing frequency on malaria transmission
# Assumes p_H = 0.5,
# by Hannah Meredith
...
period_D_range <-
  c(365 / 52, linspace(365 / 12, 365, n = 12)) #n =12
halflife_d_range <- logspace(-1, 2, n = 10) #n =10
D_death_range <-
  c(7.5, 20, 30, 65, 500, 1175)#c(10, 25, 100, 500, 1100)
m_range <- c(5, 20, 40) # m=5 is controlled with bednets in current scenario...
...
for (h in 1:size(m_range, 2)) {
  ...
  for (i in 1:size(C_l_range,2)) {
    for (j in 1:size(cmax_range,2)) {
     for (k in 1:size(halflife_d_range,2)) {
       for(l in size(period_D_range, 2):1) {
          ...
          C_l = C_l_range[i] * 1,
          C_h = C_h_range[i] * 1,
          D_death = D_death_range[1],
          hl_d = halflife_d_range[k],
          dose = cmax_range[j],
          period_D = period_D_range[l],
          m = m_range[h],
          p_h = p_h_range[2]
        )
        ...
        relative_malaria <-
          (infected_net_steadyState - tail(infected_all, n = 1)) / infected_net_steadyState * 100
        relative_malaria [relative_malaria < 0] = 0
        # relative_malaria.df <- as.data.frame(relative_malaria)
        
        print(round(c(l, k, j, i, h, relative_malaria)))
        
        if (relative_malaria >= target) {
          periodUse[k, j, i + 3*(h-1)] = period_D_range[l]
          break
        }
       }
      }
...
for (i in 1:9) {
  data <- round(periodUse[,,i])
  colnames(data) <- cmax_range
  rownames(data) <- halflife_d_range
  longData <- melt(data)
  longData.df <- data.frame(longData)
  colnames(longData.df) <- c("halflife", "cmax", "period")
  par(pty = "s") # axis
  subdataset.df <- subset(dataset.df, dataset.df$loopID == i)
  p <- ggplot(longData.df, aes(halflife, cmax))
  p1 <- p +
    stat_contour(
      aes(z = period,
          colour = stat(level)),
...
# (1) m=5, +C_L, (2) m=5, +C_h, (3) m=5, +C_L+C_h, 
# (4) m=20, +C_L, (5) m=20, +C_h, (6) m=20, +C_L+C_h, 
# (7) m=40, +C_L,  (8) m=40, +C_L,(9) m=40, +C_L 